...
* `retries`: The maximum com.pension.calculator.amount of times to retry the operation. Default is `10`. Seting this to `1` means `do it once, then retry it once`.
...
after a certain com.pension.calculator.amount of time, you can use wolfram alpha. For example in order
...
* `5 * 60 * 1000`: The desired total com.pension.calculator.amount of time for retrying in ms (5 minutes)
...
If multiple error messages occured the same com.pension.calculator.amount of time, the last error
...
Returns `false` when no `error` value is given, or the maximum com.pension.calculator.amount of retries
has been reached.
